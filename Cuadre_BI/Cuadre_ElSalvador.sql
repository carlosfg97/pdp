...
WHERE CodTipoProfit = '01' AND AnioCampana IN ('201717') and DesPalanca = 'Ofertas del Día'
...
SELECT CodEbelista, a.CodVenta, CodRegion, CodZona, *
FROM FVTAPROEBECAMC01 A INNER JOIN DMATRIZCAMPANA B ON A.AnioCampana = B.AnioCampana and a.CodVenta = b.CodVenta
inner join DTIPOOFERTA c on b.PKTipoOferta = c.PKTipoOferta
inner join DEBELISTA E on E.PkEbelista = A.PkEbelista
inner join  DGEOGRAFIACAMPANA f on f.AnioCampana = a.AnioCampana and f.PkTerritorio = a.PkTerritorio
inner join DNROFACTURA G on a.AnioCampana = g.AnioCampana and a.PKPedido = g.PKPedido
...
SELECT CodRegion, CodZona, CodEbelista, FechaEmisionFactura ,  a.CodVenta, *
FROM FVTAPROEBECAMC01 A INNER JOIN DMATRIZCAMPANA B ON A.AnioCampana = B.AnioCampana and a.CodVenta = b.CodVenta
inner join DTIPOOFERTA c on b.PKTipoOferta = c.PKTipoOferta
inner join DEBELISTA E on E.PkEbelista = A.PkEbelista
inner join  DGEOGRAFIACAMPANA f on f.AnioCampana = a.AnioCampana and f.PkTerritorio = a.PkTerritorio
...
'217','218','219','220','221','222','223','224','225','226','227','228')
and DesCatalogo = '46 - WEB OFERTAS DEL DÍA'
and a.PkEbelista not in (select PkEbelista from #NoEnDashODD)
and CodRegion = '122'
--and a.PKEbelista = '52215'

SELECT distinct a.CodVenta
FROM FVTAPROEBECAMC01 A INNER JOIN DMATRIZCAMPANA B ON A.AnioCampana = B.AnioCampana and a.CodVenta = b.CodVenta
inner join DTIPOOFERTA c on b.PKTipoOferta = c.PKTipoOferta
inner join DEBELISTA E on E.PkEbelista = A.PkEbelista
where a.AnioCampana IN ('201717') and A.AnioCampana = A.AnioCampanaRef and CodTipoProfit = '01'
...
inner join DEBELISTA E on E.PkEbelista = A.PkEbelista
where a.AnioCampana IN ('201717') and A.AnioCampana = A.AnioCampanaRef  and DesSeccion =  'Oferta del Día' 
...
WHERE CodPais = 'ES' and AnioCampanaVenta = '201717' and TipoPersonalizacion = 'ODD'
and CodEbelista = '0393362'
and CodVenta ='95592'
...
FROM FResultadoPalancas A INNER JOIN DTIPOOFERTA B ON A.CodTipoOferta = B.CodTipoOferta 
INNER JOIN DPALANCAS C ON A.PkPalanca = C.PkPalanca
WHERE CodTipoProfit = '01' AND AnioCampana IN ('201717') and DesPalanca = 'Ofertas Para tí'
...
SELECT count(distinct a.PkEbelista)
FROM FVTAPROEBECAMC01 A INNER JOIN DMATRIZCAMPANA B ON A.AnioCampana = B.AnioCampana and a.CodVenta = b.CodVenta
inner join DTIPOOFERTA c on b.PKTipoOferta = c.PKTipoOferta
inner join DEBELISTA E on E.PkEbelista = A.PkEbelista
where a.AnioCampana IN ('201717') and A.AnioCampana = A.AnioCampanaRef and CodTipoProfit = '01'
...
select  *
from DNROFACTURA
WHERE PkEbelista in (select PKEbelista from  #NoEnDashOPT) and AnioCampana = '201717'
order by FechaEmisionFactura desc
...
FROM FVTAPROEBECAMC01 A INNER JOIN DOrigenPedidoWeb D ON A.CodigoPalanca = D.OrigenPedidoWeb
inner join DEBELISTA E on E.PkEbelista = A.PkEbelista
where a.AnioCampana IN ('201717') and A.AnioCampana = A.AnioCampanaRef  and DesSeccion = 'Ofertas para ti'
...
and DesCatalogo = '45 - WEB OFERTAS PARA TI'
and a.PkEbelista in (select PkEbelista from #NoEnSBOPT)
--el descuadre no sale igual aca que en el excel
...
FROM FResultadoPalancas A INNER JOIN DTIPOOFERTA B ON A.CodTipoOferta = B.CodTipoOferta 
INNER JOIN DPALANCAS C ON A.PkPalanca = C.PkPalanca
WHERE CodTipoProfit = '01' AND AnioCampana IN ('201717') and DesPalanca = 'Showroom'


drop table #PKTOSR
SELECT distinct PKEbelista into #PKTOSR
FROM FVTAPROEBECAMC01 A INNER JOIN DMATRIZCAMPANA B ON A.AnioCampana = B.AnioCampana and a.CodVenta = b.CodVenta
--and a.PkProducto = b.PkProducto 
--and a.PkTipoOferta = b.PkTipoOferta
inner join DTIPOOFERTA c on b.PKTipoOferta = c.PKTipoOferta
where a.AnioCampana IN ('201717') and A.AnioCampana = A.AnioCampanaRef and CodTipoProfit = '01'
and C.CodTipoOferta in ('201','202','203','204','205','206','207','208','209','210','211','212','213','214','215','216',
'217','218','219','220','221','222','223','224','225','226','227','228')
and DesCatalogo = '44 - WEB SHOWROOM'
--incluyendo cualquier llave, hace que cuadre.
...
--la diferencia entre ebelistas sí se valida pero la diferencia de REALVTAMNNETO difiere con el excel.

select * from DMATRIZCAMPANA
where AnioCampana = '201717'
and CodVenta in ( '95276','95266')
order by CodVenta
...
'217','218','219','220','221','222','223','224','225','226','227','228')
and DesCatalogo = '44 - WEB SHOWROOM'
and a.PkEbelista  not in (select PkEbelista from #NoEnDashSR)

SELECT  distinct a.PkTipoOferta, b.PKTipoOferta, c.PkTipoOferta
FROM FVTAPROEBECAMC01 A INNER JOIN DMATRIZCAMPANA B ON A.AnioCampana = B.AnioCampana and a.CodVenta = b.CodVenta 
...
SELECT distinct a.PkTipoOferta, b.PKTipoOferta, c.PkTipoOferta, c.CodTipoOferta, c.DesTipoOferta
FROM FVTAPROEBECAMC01 A INNER JOIN DMATRIZCAMPANA B ON A.AnioCampana = B.AnioCampana and a.CodVenta = b.CodVenta
inner join DTIPOOFERTA c on b.PKTipoOferta = c.PKTipoOferta
inner join DEBELISTA E on E.PkEbelista = A.PkEbelista
inner join  DGEOGRAFIACAMPANA f on f.AnioCampana = a.AnioCampana and f.PkTerritorio = a.PkTerritorio
...
SELECT * FROM DTIPOOFERTA 
where PkTipoOferta in ('30','47','35')
...
WHERE aniocampana = '201717' and PKEbelista in ( select PKEbelista from #NoEnDashSR)

select CodRegion, CodZona, * 
from DNROFACTURA a inner join DGEOGRAFIACAMPANA b on a.AnioCampana = b.AnioCampana and a.PkTerritorio = b.PkTerritorio
WHERE a.aniocampana = '201717' and PKEbelista in ( select PKEbelista from #NoEnDashSR)
...
FROM FVTAPROEBECAMC01 A INNER JOIN DOrigenPedidoWeb D ON A.CodigoPalanca = D.OrigenPedidoWeb
inner join DEBELISTA E on E.PkEbelista = A.PkEbelista
where a.AnioCampana IN ('201717') and A.AnioCampana = A.AnioCampanaRef  
and DesSeccion in ('ShowRoom', 'Home','Landing Compra', 'Landing Intriga','Product Page')
and DesZona not in ('Los Mas Vendidos','RD Landing')
...
select DISTINCT DesZona, DesSeccion from DOrigenPedidoWeb
where DesSeccion in ('Home','Landing Compra', 'Landing Intriga','Product Page')
